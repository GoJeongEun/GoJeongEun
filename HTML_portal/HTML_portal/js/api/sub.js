  ...
  $('.st_list_popup_wrap.pop_type03').fadeOut();
      ...
      function faq_list() {
        ...
        $faq_btn.click(function() {
          ...
          // 통계표설명_URL 세부정보 입력 테이블 전환
          $('.radio_item > label > input').click(function(){
            $('.detail_table').addClass('active');
            $('.detail_table.active table td').css("display", "none");
            $("." + $(this).attr("id")).css("display", "");
            $('.fotmatTd').css("display", "");
         });

         //팝업 탭전환 
      $('.tab_depth2').click(function(){
        $('.tab_depth2 > button').removeClass('tab_on')
        $(this).children('button').addClass('tab_on')
      });
